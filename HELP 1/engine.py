...
import menu as dm # simple module to make a menu, cudn't make a menu so used a opensource module for that
...
def load_map(path): # load the game map according to level array
    f = open(path)
    data = f.read()
    f.close()
    data = data.split('\n')
