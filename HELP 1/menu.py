...
def menu(screen, menu, x_pos = 100, y_pos = 100, font = None,
	...
	HOW TO INTERACT
	===============
	After calling Menu(), the User MUST choose an Menupoint. The
	Script will be haltet until the User makes a decision or a event
	called pygame.QUIT() will be raised.
	
	User can press a Key from 1 to 9 to take the choice.
	...
	Drawing Menu will be antialiased. 
	...
	pygame.font.init()
	if font == None:
		theFonting = pygame.font.Font(None, size)
	else:
		theFonting = pygame.font.SysFont(font, size)
	cursorpos = 0
	renderWithChars = False
	for i in menu:
		if renderWithChars == False:
			text =  theFonting.render(str(cursorpos + 1)+".  " + i,
				True, fgcolor)
		else:
			text =  theFonting.render(chr(char)+".  " + i,
				True, fgcolor)
			char += 1
	...
	# Draw the ">", the Cursor
	cursorpos = 0
	cursor = theFonting.render(">", True, cursorcolor)
